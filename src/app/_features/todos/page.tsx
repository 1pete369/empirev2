...
const mapFetchedTodoToTodo = (fetchedTodo: FetchedTodo): Todo => {
  return {
    ...
    status: (["completed", "pending", "overdue"].includes(fetchedTodo.status as "completed" | "pending" | "overdue"))
...
function reducer(state: Todo[], action: Action): Todo[] {
  switch (action.type) {
    case "LoadTodos" : return Array.isArray(action.todos) ? action.todos.map(mapFetchedTodoToTodo) : state
    case "AddTodo":
      ...
      <div className="mt-4 max-h-[400px] max-w-full overflow-y-auto">
        <div className="w-full flex flex-col gap-2">
          <div className="w-full flex gap-1 bg-slate-200">
            <div className="p-2 text-left w-1/2">Todo</div>
            <div className="p-2 text-center w-[40px] ">✅</div>
          ...
          {state.length > 0 ? (
            state.map((todo) => {
              return (
                <div
                  ...
                  key={todo.id}
                >
                  <div className={`p-2 text-left w-1/2  break-words ${todo.completed && " line-through"}`}>
                    {todo.name}
                  </div>
            ...
            <div className="block text-center"> {isLoading ? "Loading...":"No todos!"}</div> // Show this after the fetch is complete
          )}
        </div>
      </div>
    </div>
